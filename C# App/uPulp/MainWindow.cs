...
namespace keybon
{
    ...
    public partial class MainWindow : Form
    {
        ...
        public MainWindow()
        {
            ...
            Timer timer1 = new Timer{ Interval = 200 };
            timer1.Enabled = true;
        ...
        private void portDataReceived(object sender, EventArgs args)
        {
            ...
            try
            {
                if (keyReceived == 49) SendKeys.SendWait("a");
                if (keyReceived == 50) SendKeys.SendWait("s");
                if (keyReceived == 51) SendKeys.SendWait("{UP}");
                if (keyReceived == 52) SendKeys.SendWait("{DOWN}");
                if (keyReceived == 53) SendKeys.SendWait("{LEFT}");
                if (keyReceived == 54) SendKeys.SendWait("{RIGHT}");
            }
            catch { }
        ...
        {
            //Create a new bitmap.
            var bmpScreenshot = new Bitmap(400, 240, PixelFormat.Format32bppArgb);

            // Create a graphics object from the bitmap.
            var gfxScreenshot = Graphics.FromImage(bmpScreenshot);

            // Take the screenshot from the upper left corner to the right bottom corner.
            gfxScreenshot.CopyFromScreen(1080, 191, 0, 0, bmpScreenshot.Size, CopyPixelOperation.SourceCopy);


            Layouts[0].writeImageToSerial(_serialPort, 1, bmpScreenshot);

            // Save the screenshot to the specified path that the user has chosen.
            //bmpScreenshot.Save("C:\\Users\\Maximilian\\Desktop\\screenshot.png", ImageFormat.Png);
    ...
    public class ScreenLayout
    {
        ...
        public void writeImageToSerial(SerialPort _serialPort, int keyNum, Bitmap bmp)
        {
